...
export default function ImprimirComprobantePage() {
  ...
  const openPopupWithHtml = () => {
    if (!html) return;
    const popup =
      popupRef.current && !popupRef.current.closed
        ? popupRef.current
        : window.open('', '_blank', 'width=960,height=900');
    if (!popup) {
    ...
    window.setTimeout(() => {
      try {
        ...
        setError('No se pudo abrir la vista previa de impresion');
      }
    }, 350);
  };
  ...
  return (
    <div className={styles.page}>
      <div className={styles.toolbar}>
        <Link href="/admin/comprobantes" className={styles.backButton}>
          Volver a comprobantes
        </Link>
        <div className={styles.actions}>
          <button type="button" className={styles.printButton} onClick={handleManualPrint} disabled={!html}>
            Imprimir / Guardar PDF
          </button>
        </div>
      </div>

      {loading ? <div className={styles.status}>Preparando comprobante para imprimir...</div> : null}
      {!loading && error ? <div className={styles.error}>{error}</div> : null}

      {html ? (
        <div className={styles.status}>
          Comprobante listo para imprimir. Usa el boton `Imprimir / Guardar PDF`.
        </div>
      ) : null}
    </div>
  );
